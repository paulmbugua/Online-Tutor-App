...
const Messages = () => {
  const { token, fetchMessages, fetchConversations, chats, markAsRead, sendMessage, userId } = useContext(ShopContext);
  ...
  // Keep activeChat in sync with updated chats
  useEffect(() => {
    if (activeChat) {
        ...
        if (updatedActiveChat && updatedActiveChat !== activeChat) {
            setActiveChat(updatedActiveChat); // Only update if data has changed
        }
    }
}, [chats]);
  ...
  const openChat = (chat) => {
    ...
    setActiveChat(chat); // Persist active chat

    const hasUnreadForRecipient = chat.messages.some(
        (msg) => msg.unread && msg.sender._id !== userId
    );

    if (hasUnreadForRecipient) {
        console.log(`Marking messages as read for recipient with ID: ${chat.recipientId}`);
        markAsRead(chat.recipientId);
    } else {
        console.log('No unread messages for this chat or user is the sender');
    }

    setMessageOffset(0);
    fetchMessages(chat.recipientId, messagesLimit, 0);
    setSidebarOpen(false); // Close the sidebar in mobile view
};
    ...
    chats.map((chat, index) => (
      <li
        ...
        className={`p-3 rounded-lg cursor-pointer transition ${
          chat.messages?.some((msg) => msg.unread && msg.sender._id !== userId) 
            ? 'bg-gray-700' 
            : 'bg-gray-800'
        ...
        <div className="flex items-center space-x-3">
          ...
          <div className="flex-grow">
            ...
            <p className="text-sm text-gray-400 truncate">
              {chat.messages && chat.messages[chat.messages.length - 1]?.content || "Start a conversation"}
            </p>
          </div>
        </div>
      </li>
          ...
          {activeChat ? (
            <div className="space-y-3">
              {activeChat.messages?.slice().sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp)).map((msg, index) => (
                <div key={index} className={`flex ${msg?.sender?._id === userId ? 'justify-end' : 'justify-start'} transition-transform`}>
