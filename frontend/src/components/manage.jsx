...
const ManageProfileForm = () => {
  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      try {
        ...
        // Safely format gallery with fallback
        ...
        console.log('Selected languages:', languageSelection);
  ...
  useEffect(() => {
    const fetchAvailableProfiles = async () => {
      try {
        ...
        // Set `availableProfiles` to the actual array of profiles
  ...
  const handleSearch = (e) => {
    ...
    // Check if `availableProfiles` is an array before calling `.filter()`
  ...
  const handleDeleteImage = async (index) => {
    try {
      ...
      await axios.delete(`${backendUrl}/api/profile/${profile._id}/remove/gallery`, {
        headers: { Authorization: `Bearer ${token}` },
        data: { item: imageUrl }, // Send the image URL as the item
      });
      setUpdatedData((prev) => {
        const updatedGallery = [...prev.gallery];
        updatedGallery[index] = null; // Remove the image from the gallery
  ...
  const handleDeleteVideo = async () => {
    try {
      await axios.delete(`${backendUrl}/api/profile/${profile._id}/remove/video`, {
        headers: { Authorization: `Bearer ${token}` },
        data: { item: updatedData.video }, // Send the video URL as the item
      });
  ...
  const handleSubmit = async (e) => {
    ...
    const handleTeachingStyleSelect = (style) => {
      setUpdatedData((prev) => {
        const teachingStyles = [...prev.teachingStyle]; // Copy the current array
        if (teachingStyles.includes(style)) {
          // Deselect: Remove the style if it's already selected
          return { ...prev, teachingStyle: teachingStyles.filter((item) => item !== style) };
        } else {
          // Select: Add the style to the array
    ...
    const handleAgeGroupSelect = (group) => {
      setUpdatedData((prev) => ({
        ...
        ageGroup: prev.ageGroup.includes(group)
          ? prev.ageGroup.filter((item) => item !== group) // Deselect if already selected
          : [...prev.ageGroup, group], // Add to the array if not selected
      }));
    };
  ...
  return<form onSubmit={handleSubmit} className="space-y-4 p-4 bg-gray-900 rounded-lg shadow-lg max-w-lg mx-auto pb-20">
    ...
    <>
      ...
      <div className="space-y-2">
        ...
        <textarea
          ...
          onChange={handleInputChange}
          placeholder="Write a brief introduction about yourself, including your teaching philosophy, achievements, or anything that sets you apart as a tutor."
          className="w-full p-2 rounded bg-gray-800 text-white"
          rows={3}
        />
      </div>
  ...
  <div className="w-40 h-40 border flex items-center justify-center relative group">
    ...
    <div className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-50 opacity-0 group-hover:opacity-100 transition-opacity">
      {updatedData.gallery[0] && (
        <button
          onClick={() => handleDeleteImage(0)} // Targets the first image
      ...
      <label className="p-2 bg-blue-500 text-white rounded cursor-pointer">
        ...
        <input
          type="file"
          hidden
          onChange={(e) => handleFileChange(e, 0, 'image')} // Adjust for single image
        />
      </label>
    </div>
  </div>
