...
const AccountSection = () => {
  const { token, backendUrl, tokens, userEmail, setTokens } = useContext(ShopContext);
  ...
  const handleConfirmComplete = async (sessionId) => {
    try {
      ...
      console.log('🔹 Session Confirmed as Complete:', response.data);
      alert('Session confirmed as complete.');
  
      // ✅ Wait for the session status update in the API response before updating state
      ...
      const completedSession = accountDetails.session?.find(s => s._id === sessionId);
      if (completedSession) {
        setRatingData({
          tutorId: completedSession.tutorId?._id || completedSession.tutorId,
          sessionId: sessionId,
          rating: '',
          comment: ''
        });
  ...
  const handleReviewSubmission = async () => {
    try {
      ...
      const response = await axios.post(
        `${backendUrl}/api/tutor-session/review/create`,
